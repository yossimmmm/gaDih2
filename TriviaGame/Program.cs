...
{
    // טוען הגדרות SMTP מה-Configuration/ENV ומרכיב אובייקט אחיד לשירות המייל.
...
{
    // כל אירוע אימות (login/reset וכו') נרשם ללוג מרכזי לצורכי מעקב והצגה בבחינה.
...
app.Use(async (context, next) =>
{
    ...
    if (protectedPrefixes.Any(p => path.StartsWith(p, StringComparison.OrdinalIgnoreCase)))
    {
        ...
        // תרגום הטוקן למזהה משתמש חוקי מתוך טבלת sessions.
...
app.MapPost("/api/auth/login", async (HttpContext http, LoginRequest req, AuthAuditDispatcher audit) =>
{
    ...
    var sessionDb = new SessionDB();
    // יצירת טוקן סשן בצד שרת (נשמר ב-DB עם תוקף).
    var token = await sessionDb.CreateSessionAsync(user.UserID, TimeSpan.FromDays(7));

    // כתיבת session_token ל-cookie מאובטח; זה הבסיס לאימות בכל בקשה עתידית.
...
app.MapGet("/api/auth/me", async (HttpContext http) =>
{
    ...
    var userDb = new UserDB();
    // מחזיר userId + role כדי ש-Blazor ישחזר מצב Session בצד לקוח.
...
app.MapPost("/api/auth/forgot-password", async (HttpContext http, ForgotPasswordRequest req, EmailService emailService, AuthAuditDispatcher audit) =>
{
    ...
    // בניית קישור מלא שחוזר למסך reset-password עם token כ-query.
...
// endpoint אדמין: שליפת רשימת משתמשים לניהול
app.MapGet("/api/admin/users", async (HttpContext http) =>
{
    // אימות שהמשתמש המחובר הוא אדמין
    ...
    // עיצוב payload נקי להחזרה ללקוח
...
// endpoint אדמין: שינוי תפקיד משתמש
app.MapPost("/api/admin/users/{userId:int}/role", async (HttpContext http, int userId, RoleUpdateRequest req) =>
{
    ...
    if (!Enum.TryParse<UserRole>(req.Role, ignoreCase: true, out var parsedRole))
        return Results.BadRequest(new { ok = false, message = "Role must be User, Manager, or Admin." });
...
{
    // אימות משתמש נוכחי והרשאת אדמין בלבד
    var current = await GetCurrentUserAsync(http);
    if (current is null || current.Role != UserRole.Admin)
        return Results.Unauthorized();

    // בדיקת מזהה משתמש תקין
    if (userId <= 0)
        return Results.BadRequest(new { ok = false, message = "Invalid user id." });

    // בדיקת אימייל תקין לפני עדכון
    ...
    // המרת טקסט תפקיד ל-enum עם ולידציה
    if (!Enum.TryParse<UserRole>(req.Role, ignoreCase: true, out var parsedRole))
        return Results.BadRequest(new { ok = false, message = "Role must be User, Manager, or Admin." });

    // ביצוע עדכון נתוני משתמש במסד
    var userDb = new UserDB();
...
// נקודת חיבור realtime לכל אירועי חדר/לובי/משחק.
...
static SmtpSettings BuildSmtpSettings(IConfiguration cfg)
{
    ...
    // ברירת מחדל: אם parsing נכשל, secure יישאר true כדי לא לשלוח SMTP פתוח.
...
{
    // דפי auth/קבצי סטטיים/API לא דורשים redirect ללוגין במידלוור הזה.
    if (path == "/")
        return true;
...
{
    // בדיקת קיום session_token בעוגיות
    if (!http.Request.Cookies.TryGetValue("session_token", out var token) || string.IsNullOrWhiteSpace(token))
        return null;

    // המרת טוקן למזהה משתמש דרך טבלת sessions
    var sessionDb = new SessionDB();
    var userId = await sessionDb.GetUserIdByTokenAsync(token);
    if (!userId.HasValue)
        return null;
