...
1. השרת רץ בפרויקט `TriviaGame` על פורט `5297`.
2. במסך המובייל המשתמש מזין כתובת Backend (לדוגמה `http://10.0.2.2:5297`).
...
- אמולטור Android: `http://10.0.2.2:5297`
- Windows על אותו מחשב: `http://localhost:5297`
- טלפון אמיתי באותה רשת: `http://<כתובת-LAN-של-המחשב>:5297`

## הערות חשובות

- בטלפון אמיתי אי אפשר להשתמש ב-`localhost`.
- אם חומת אש חוסמת, יש לפתוח גישה לפורט `5297`.
