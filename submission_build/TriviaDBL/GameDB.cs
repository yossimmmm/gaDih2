    ...
    public class GameDB
    {
        ...
        public async Task<int> PickQuestionsForRoomAsync(int roomId, int count)
        {
            ...
            try
            {
                ...
                try
                {
                    // Clear old answers for this room
                    {
...
LIMIT @cnt;";
                    ...
                    // Insert room_questions in order
                    int inserted = 0;
...
VALUES (@room_id, @qid, @ord, 15);";
        ...
        public async Task<bool> SubmitAnswerAsync(int roomPlayerId, int questionId, int optionId)
        {
            ...
            try
            {
                ...
                {
                    int roomId;
                    // Find room_id for this player (also ensures player exists)
                    {
                        const string rpSql = @"
SELECT room_id
FROM room_players
...
LIMIT 1;";
                    ...
                    // Verify option belongs to question and get is_correct
                    bool isCorrect;
                    {
                        const string chkSql = @"
SELECT is_correct
...
LIMIT 1;";
                    ...
                    // Delete any previous answer for that player+question
                    {
                        const string delSql = @"
